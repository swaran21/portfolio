  ...
  return (
    <div className="w-full px-5 md:px-16 max-w-[1400px] mx-auto text-left drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-8 md:p-10 border border-primary/30 bg-background/85 flex flex-col justify-between">
          <div>
            <p className="font-body text-sm text-on-surface font-semibold leading-relaxed mb-8 uppercase tracking-wide drop-shadow-[0_1px_1px_rgba(0,0,0,1)] opacity-90">
        ...
        <div className="glass-panel p-8 md:p-10 border border-primary/30 bg-background/85 relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8 mt-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 pt-4">
              <button 
                ...
                className="w-full sm:w-auto px-10 py-4 bg-background/80 border border-primary text-primary font-label-caps text-xs tracking-widest font-bold hover:bg-primary hover:text-background hover:glow-sm transition-all duration-300 flex items-center justify-center gap-4 cursor-pointer"
              >
                EXECUTE_TRANSMISSION
                <span className="material-symbols-outlined text-[18px]">send</span>
              </button>
              
              <span className="font-body text-[10px] text-on-surface font-bold uppercase tracking-tight drop-shadow-[0_1px_1px_rgba(0,0,0,1)] bg-background/50 px-2 py-1">
      ...
      {sentMessages.length > 0 && (
        <div className="mt-12 border border-primary/30 p-6 glass-panel bg-background/85">
          ...
          <div className="space-y-4 max-h-[200px] overflow-y-auto pr-2">
            {sentMessages.map((msg) => (
              <div key={msg.id} className="p-4 border border-primary/20 bg-background flex flex-col md:flex-row justify-between md:items-center gap-2">
                <div>
                  <span className="font-label-caps text-[9px] text-primary font-bold block drop-shadow-md">{msg.channel} // {msg.timestamp}</span>
                  <p className="font-body text-xs text-on-surface font-semibold mt-1 drop-shadow-[0_1px_1px_rgba(0,0,0,1)]">{msg.content}</p>
                </div>
                <span className="font-label-caps text-[9px] text-on-surface font-bold border border-primary/40 bg-primary/10 px-2 py-0.5 self-start md:self-center drop-shadow-md">
